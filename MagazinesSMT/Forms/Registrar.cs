...
namespace MagazinesSMT.Forms
{
    public partial class Registrar : Form
    {
        ...
        private void btnGenerarMagazine_Click(object sender, EventArgs e)
        {

            string serialMagazine = GenerarSerialMagazine();

            bool insercion = database.InsertarNuevoMagazine(serialMagazine);

            if (insercion)
            {

                MessageBox.Show("Magazine creado!!");

                btnGenerarMagazine.Enabled = false;

                lblSerialMagazine.Text = serialMagazine;
            ...
            else
            {
                ...
                MessageBox.Show("Error al insertar el magazine");
            }
        ...
        {         
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                int idMagazine = database.obtenerIDMagazine(lblSerialMagazine.Text);
                    ...
                    if (txtSerial.Text.Trim() != string.Empty)
                    {
                        ...
                        if (error == 0)
                        {
                            ...
                            if (insertado)
                            {
                                ...
                                listaSeriales = database.obtenerDatosDeMagazine(lblSerialMagazine.Text,out statusMagazine);
        ...
        // Validar si el magazine esta lleno
        ...
        public void ReiniciarMagazine() {
            ...
            // Limpiamos magazine por llenar
            txtMagazine.Text = string.Empty;
            // Disable el boton de generar magazine
            btnGenerarMagazine.Enabled = false;
        ...
        private void cbbOpcion_SelectedIndexChanged(object sender, EventArgs e)
        {

            if (cbbOpcion.Text == "Generar Nuevo Magazine")
            {
                txtMagazine.Enabled = false;
                btnGenerarMagazine.Enabled = true;
            }
            else if (cbbOpcion.Text == "Completar Magazine")
            {
                btnGenerarMagazine.Enabled = false;
                txtMagazine.Enabled = true;
                lblMagazine.Enabled = true;
            }
        ...
        {
            BindingList<RegistroSerial> listaMagazine = new BindingList<RegistroSerial>();

            if (e.KeyCode == Keys.Enter)
            {
                string statusMagazine = string.Empty;
                // Recuperar el los datos del magazine
                listaMagazine =  database.obtenerDatosDeMagazine(txtMagazine.Text.Trim(),out statusMagazine);

                if (listaMagazine.Count > 0)
                {
                    dataGridView1.DataSource = listaMagazine;

                    txtMagazine.Enabled = false;
                    txtSerial.Enabled = true;
                    lblSerialMagazine.Text = txtMagazine.Text.Trim();
                    label5.Text = statusMagazine;
                    // Contamos filas
                    ContarFilas();
                }
                ...
                {
                    MessageBox.Show("No se encontraron datos del magazine");
                }
